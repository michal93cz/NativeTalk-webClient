...
export class NoticeService {
  ...
  getNotices(type: string, language?: string, city?: string): Promise<Notice[]> {
    let url = this.noticesUrl + `?type=${type}&language=${language}&city=${city}`;
  ...
  getMyApplies(): Promise<Notice[]> {
    let url = this.noticesUrl + `/myapplies`;
    return this.http.get(url, {headers: this.headers})
      .toPromise()
      .then(response => response.json() as Notice[])
      .catch(this.handleError);
  ...
  apply(id: string): Promise<void> {
    const url = `${this.noticesUrl}/${id}/apply`;
    return this.http.get(url, {headers: this.headers})
      .toPromise()
      .then(() => null)
      .catch(this.handleError);
